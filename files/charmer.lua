...
if player_id then
    ...
    if genome_component then --Copies relations from from_faction to poly_player. Definitely not the best way to code this but I just think it's neat.
        ...
        local from_faction = genome2D[1]
        [ComponentGetValue2(genome_component, "herd_id") + 2] -- The GetValue here gets the index and not the name. Also, the index starts at -1... This took hours to figure out because I am a fool.
        for x = 2, #genome2D[1] do -- replace poly_player row with from_faction row, skipping the faction name.
            ---@diagnostic disable-next-line: param-type-mismatch
            genome2D[row["poly_player"]][x] = math.max(tonumber(genome2D[row["player"]][x]),
                tonumber(genome2D[row[from_faction]][x])) -- Use the higher of player/from_faction relations so relations never decrease
        end
        for y = 2, #genome2D do -- replace poly_player column with from_faction column, skipping the faction name.
            ---@diagnostic disable-next-line: param-type-mismatch
            genome2D[y][column["poly_player"]] = math.max(tonumber(genome2D[y][column["player"]]),
                tonumber(genome2D[y][column[from_faction]])) -- Use the higher of player/from_faction relations so relations never decrease
